    ...
    for (let { branch, event, identifiers } of history) {
      ...
      if (eventModel) {
        ...
        if (!contractDefinition.eventContentTriggers[eventModel.attributes['event-name']].length) {
          let contract = await this.searchers.getResourceAndMeta(Session.INTERNAL_PRIVILEGED, branch, pluralize(contractName), contractAddress).resource;
          if (!contract) {
      ...
      for (let { id, type, isContractType } of identifiers) {
        ...
        let existingRecord = await this.searchers.getResourceAndMeta(Session.INTERNAL_PRIVILEGED, branch, type, id.toLowerCase()).resource;
        if (existingRecord) {
